/**
 * 指定日の各種リスティング情報を都道府県ごとに収集し、kintoneに送信する
 */
function main() {
  ...
  const dateStrings = dates.map(targetDate=>`${targetDate.getFullYear()}-${(targetDate.getMonth()+1).toString().padStart(2, '0')}-${(targetDate.getDate()).toString().padStart(2, '0')}`);

  for (const dateString of dateStrings) {
    dataCollection.push(...getAllPrefStats(dateString));
    dataCollection.push(...getAllNationalStats(dateString));
    dataCollection.push(...getAllYoutubeStats(dateString));
  }
  const totalledResults=Object.values(dataCollection.reduce((acc,cur)=>{
    // const key = `${cur.area.value}`;
    const key = `${cur.date.value}_${cur.platform.value}_${cur.campaign.value}_${cur.campaign_type.value}_${cur.area.value}`;
    if (key in acc) {
      acc[key].cost.value += cur.cost.value;
      acc[key].clicks.value += cur.clicks.value;
      acc[key].impressions.value += cur.impressions.value;
      acc[key].conversions.value += cur.conversions.value;
    } else {
      acc[key] = cur;
    }
    return acc;
  ...
  Logger.log(JSON.stringify(totalledResults));

  for (let offset = 0; offset < totalledResults.length; offset += 100) { // 100件ずつ送信(kintone一括登録上限)
    Utilities.sleep(50); // rate limit避け
    postToKintone(
      {
        app: 505,
        records: totalledResults.slice(offset, offset + 100)
      },
      basic,
      token
    );
